...
const BASE_URL = 'http://RWServerPhone/'
...
class PhoneAPI {
  constructor () {
    window.addEventListener('message', (event) => {
      ...
      } else if (event.data.type === 'setPermissions') {
        this.permissions = []
        for (let key in event.data.type) {
          this.permissions.push(event.data.type[key])
        }
      }
    })
    this.permissions = []
  ...
  async notesGetMessagesChannel (channel) {
    ...
    this.post('notes_getChannel', { channel })
  }
  async notesSendMessage (channel, message) {
    this.post('notes_addMessage', { channel, message })
  }

  async sendControlAction (params) {
    return this.post('control', { params })
  }
