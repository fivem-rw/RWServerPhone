...
export default {
  ...
  methods: {
    ...
    async onRight () {
      ...
      let choix = [
        {id: 1, title: this.IntlString('APP_NOTES_NEW'), icons: 'fa-plus', color: 'dodgerblue'},
        {id: 2, title: this.IntlString('APP_NOTES_DELETE'), icons: 'fa-minus', color: 'tomato'},
    ...
    async addChannelOption () {
      try {
        ...
        if (channel.length > 0) {
          let isDup = false
          for (let key in this.notesChannels) {
            if (this.notesChannels[key].channel === channel) {
              isDup = true
              break
            }
          }
