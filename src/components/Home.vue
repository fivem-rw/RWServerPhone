  ...
  <div style="width: 326px; height: 743px;" class="home" v-bind:style="{background: 'url(' + backgroundURL +')'}">
    ...
    <div class='app_buttons'>
      ...
      <button style=" top: 73px; font-family:initial; margin-left: 10px; margin-right: 10px;"
          ...
          v-show="!but.isBottom" 
          v-bind:key="but.name" 
          v-bind:class="{ select: key === currentSelect, isname:true}"
...
export default {
  ...
  computed: {
    ...mapGetters(['IntlString', 'useMouse', 'nbMessagesUnread', 'backgroundURL', 'messages', 'AppsHome', 'warningMessageCount', 'homeCurrentSelect'])
  },
  methods: {
    ...mapActions(['closePhone', 'setMessages', 'getHomeCurrentSelect', 'setHomeCurrentSelect']),
    onLeft () {
    ...
    onUp () {
      this.currentSelect = Math.max(this.currentSelect - 3, 0)
    },
    onDown () {
      this.currentSelect = Math.min(this.currentSelect + 3, this.AppsHome.length)
    },
    openApp (app) {
    ...
    onEnter () {
      ...
      this.setHomeCurrentSelect(this.currentSelect)
    },
    onBack () {
      this.closePhone()
    }
  ...
  created () {
    ...
    this.currentSelect = this.homeCurrentSelect
  },
  beforeDestroy () {
