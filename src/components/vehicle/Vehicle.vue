<template>
  <div class="phone_app">
    <PhoneTitle :title="IntlString('APP_VEHICLE_TITLE')" @back="onBackspace"/>
...
export default {
  ...
  computed: {
    ...
    paramList () {
      ...
      const confirmResetStr = this.IntlString('APP_VEHICLE_RESET_CONFIRM')
      const cancelOption = {}
      const confirmReset = {}
      ...
      return [
        ...
        },
        {
          ...
          onValid: 'setControlAction',
          values: {
            'ch_va_detach_towtruck': '견인차 분리',
            'ch_va_detach_cargobob': '카고밥 분리',
            'ch_va_detach_trailer': '트레일러 분리'
          }
        }
      ]
    },
  ...
  methods: {
    ...
    setControlAction: function (params, data) {
      ...
      if (data.value === 'ch_repair' || data.value === 'ch_va_trunk') {
